...
public class ThomasEmanuelBermúdezMora_Bono_Tarea4 {
    ...
    static void menu() throws NumberFormatException, IOException {
        ...
        do {
            ...
            out.print("Opciones: \n1) Calcular mes con más consumo\n2) Calcular la eco-factura del mes\n3) Calcular la eco-factura comercial del mes\n4) Salir\n\nPor favor ingrese opción que desee (1-4): ");
            ...
            switch (opcionMenu) {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    solicitarDatosEcoFacturaComercial();
                    break;
                default:
    ...
    static Double montoEnergia_FacturaComercial(int kwhEnergia){
        //Variable de salida
        Double montoFacturadoEnergia = 0.0;
        if(kwhEnergia <= 3000 && kwhEnergia > 0){
            montoFacturadoEnergia = 221160.00;
        }else{
            montoFacturadoEnergia = 221160.00+((kwhEnergia-3000)*73.72);
        }
        return montoFacturadoEnergia;
    }

    static Double montoPotencia_FacturaComercial(int kwPotencia){
        //Variable de salida
        Double montoFacturadoPotencia = 0.0;
        if(kwPotencia <= 8 && kwPotencia > 0){
            montoFacturadoPotencia = 92311.68;
        }else{
            montoFacturadoPotencia = 92311.68+((kwPotencia-8)*11538.96);
        }
        return montoFacturadoPotencia;
    }

    static void solicitarDatosEcoFacturaComercial() throws IOException{
        //Variable de entrada
        int kwhMes, kwPotencia;
        String mesFactura;
        
        out.println("\n--------------------------- Opcion 3: Calcular Eco-Factura comercial ---------------------------\n");
        out.println("Para calcular su eco factura ingrese los siguientes datos:\n");
        out.print("Nombre del mes a facturar: ");
        mesFactura = in.readLine();
        out.print("Kilowatts por hora consumidos en "+mesFactura.toLowerCase()+": ");
        kwhMes = Integer.parseInt(in.readLine());
        out.print("Kilowatts consumidos por potencia en "+mesFactura.toLowerCase()+": ");
        kwPotencia = Integer.parseInt(in.readLine());

        imprimirEcoFacturaComercial(mesFactura, kwhMes, kwPotencia);
    }

    static void imprimirEcoFacturaComercial(String mesFactura, int kwhEnergia, int kwPotencia) {
        //Variable de salida
        Double  montoFacturadoEnergia = montoEnergia_FacturaComercial(kwhEnergia),
                montoFacturadoPotencia = montoPotencia_FacturaComercial(kwPotencia),
                montoAlumbrado = montoAlumbradoPublico(kwhEnergia),
                montoTributo = montoTributoBomberos(kwhEnergia),
                montoIva = montoIva(kwhEnergia, montoFacturadoEnergia),
                totalFactura= montoFacturadoEnergia + montoFacturadoPotencia + montoAlumbrado + montoTributo + montoIva;

        out.println("\n------------------------------------------------------------------------------------------");
        out.println("Desglose Eco-Factura comercial\n");
        out.println("Mes facturado: "+ mesFactura.toUpperCase());

        out.println("Total de KWH consumidos en "+mesFactura.toLowerCase()+": " + kwhEnergia);
        out.println("Total de KW consumidos por potencia en "+mesFactura.toLowerCase()+": " + kwPotencia);
        out.printf("\nMonto energía: %.2f", montoFacturadoEnergia);
        out.printf("\nMonto potencia: %.2f", montoFacturadoPotencia);
